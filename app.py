...
# 当日の試合情報を事前に自動判定
today_games, _ = fetch_schedule_and_starters(date_str)

if not today_games:
    st.warning(f"⚠️ 選択された日付（{date_jp_str}）はNPBの試合開催予定がないか、日程情報を取得できませんでした。")
...
if mode == "1試合ごとの個別分析":
    ...
    # 試合がある場合は自動的に試合数とカードをセット、無い場合は手動設定
    ...
    for i in range(int(num_games)):
        ...
        # 試合カードが存在する場合は自動選択
        default_home_idx = ALL_NPB_TEAMS.index(today_games[i][0]) if i < len(today_games) and today_games[i][0] in ALL_NPB_TEAMS else (i % 12)
        default_away_idx = ALL_NPB_TEAMS.index(today_games[i][1]) if i < len(today_games) and today_games[i][1] in ALL_NPB_TEAMS else ((i + 6) % 12)

        with g_col1:
            h_team = st.selectbox(f"ホーム #{i+1}", ALL_NPB_TEAMS, index=default_home_idx, key=f"bulk_h_{i}")
        with g_col2:
        ...
        with g_col4:
            a_team = st.selectbox(f"ビジター #{i+1}", ALL_NPB_TEAMS, index=default_away_idx, key=f"bulk_a_{i}")
