...
"""
NPB分析・予測スコア算出エンジン (全投手オンライン検索対応版)
"""
from scraper import fetch_pitcher_stats_online
...
def calculate_expected_score(home_team: str, away_team: str, home_pitcher_name: str, away_pitcher_name: str) -> tuple[float, float, dict]:
	home_info = TEAM_DATA.get(home_team, TEAM_DATA["阪神タイガース"])
	away_info = TEAM_DATA.get(away_team, TEAM_DATA["読売ジャイアンツ"])
    
	# 画面で入力された投手名を使ってWeb上から最新成績を検索・取得
	home_starter = fetch_pitcher_stats_online(home_pitcher_name)
	away_starter = fetch_pitcher_stats_online(away_pitcher_name)
